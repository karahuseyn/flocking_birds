...
MAXS = 30; PAD = 10; K = 11                                       # cap, pad class, num classes
...
def forward(P, class_canvas):                                    # (N,S,S) ints -> logits (N*S*S,K)
    N, S, _ = class_canvas.shape; x = T(EMB[class_canvas])        # (N,S,S,8)
...
def augment(pairs, S):
    ...
    for gi, go in pairs:
        for fn in DIH:
            if fn(gi).shape[0] <= S and fn(gi).shape[1] <= S and fn(go).shape[0] <= S and fn(go).shape[1] <= S:
                ins.append(canvas(fn(gi), S)); outs.append(canvas(fn(go), S))
    return np.array(ins), np.array(outs)

def train_task(pairs, S, steps=200, C=24, L=2, lr=3e-3, seed=0):
    np.random.seed(seed)
    P = model_params(C, L); ps = [v for k, v in P.items() if isinstance(v, T)]
    opt = Adam(ps, lr); Xin, Xout = augment(pairs, S); tgt = Xout.reshape(-1); mask = np.ones_like(tgt, float)
    for _ in range(steps):
        opt.zero(); logits = forward(P, Xin); loss = softmax_ce(logits, tgt, mask); loss.backward(); opt.step()
    return P

def predict(P, g, S):                                            # test-time augmentation voting
    H, W = A(g).shape; votes = np.zeros((H, W, K))
    for d, fn in enumerate(DIH):
        gi = fn(A(g))
        if gi.shape[0] > S or gi.shape[1] > S: continue
        lg = forward(P, canvas(gi, S)[None]).data.reshape(S, S, K)
        pred = lg.argmax(-1)[:gi.shape[0], :gi.shape[1]]
        back = DIH[INV[d]](pred)
        if back.shape == (H, W):
            for r in range(H):
                for c in range(W): votes[r, c, back[r, c]] += 1
    return votes.argmax(-1)

def solve(task, steps=200):
    pairs = [(A(p["input"]), A(p["output"])) for p in task["train"]]
    allg = [g for pr in pairs for g in pr] + [A(t["input"]) for t in task["test"]]
    S = min(MAXS, max(max(g.shape) for g in allg))
    if S > MAXS: return None
    P = train_task(pairs, S, steps=steps)
    return [A(predict(P, tp["input"], S)) for tp in task["test"]]
